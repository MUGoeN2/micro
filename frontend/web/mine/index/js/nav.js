...
$(function () {
    ...
    (function () {
        ...
        for (var i = 0; i < len; i++) {
            ...
            var temp=link_page.attr('href').replace(/(?:_\d)?\..*/, '');   //替换成 'manage?cate=1'
            var rep=temp.match(/\?.*/);                                    //匹配出 '?cate=1'
            var url_end=temp.replace(rep,'');                              //替换成  manage
            if(url_end!='index'){
                if (href.indexOf(url_end) > 0) {    // 静态页面用
