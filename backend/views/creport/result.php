            ...
            <tr>
                ...
                foreach($finalReport as $v){      //遍历得到的报告名
                        ...
                        if(!in_array($item,$menu)){
                            if($i==5) $i=0;
                            if(!empty($result)) {
                                echo "<td style='background-color:$arr[$i]; '><a href='$url'>$item</a></td>";  //大类标题以及链接
                            }else{
...
foreach($finalReport as $v){
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-md-12 text-center">
                    ...
                    if(!empty($result)){
                        ?>
                        <a href="<?php echo \yii\helpers\Url::to(['creport/detail','paper_id'=>$paper_id,'level'=>$item,'name'=>$v->name,'product_id'=>$product_id]);?>"><label style="padding:2px 15px;font-size:15px;background-color:<?php echo $arr[$i]; ?>;">点击 了解详细结果</label></a>
                    <?php }else{?>
    ...
    if(empty($result)){   //如果没有付费则 只显示一小部分
        break;
    }
...
<?php  if(empty($result)){   //如果没有付费则 显示可支付界面?>
        ...
        <div style="border: 1px solid #f6f6f6">
            ...
            <div class=" text-center alert alert-default" style="background-color: #e4e4e4">
                <ul class="list-inline">
                    ...
                    <li class="" style="border: none;"><a class="btn" style="font-size: 25px;color: #000000">99元</a> <button  id="buy" class="btn btn-primary">购买</button></li>
...
<script>
    $(function(){
        $('#buy').click(function(){  //点击购买
            ...
            $.post('grouth/payment?product_id='+product_id,function(data){
                trade_no=data.obj.trade_no;
                var url=data.obj.url;
                var html='<img  src="<?php echo \yii\helpers\Url::to(['qrcode'])?>?url='+url+'" width="100%" />';
    ...
    function showTime(trade_no)
    {
        ...
        $.post('grouth/query?out_trade_no='+out_trade_no,function(data){
            //  alert(data.obj.trade_state);
            if(data!=false&&data.obj.trade_state=="SUCCESS"){
                var html='<img width="50%" src="<?php echo Yii::$app->request->baseUrl.'/img/ok.png'?>"  style="margin-top:60px">';
                html+='<p>支付成功！</p>';
                $('#qrcode').html(html);
                $('#buy').text('点击查看');
                $('#buy').attr('data-target','');
                $('#buy').removeAttr('id');
                paid=true;   //停止循环
            }
        });
        ...
        // alert(temp);
        if(paid==true||temp==true) {  //如果扫码框影藏 或者已支付成功  则停止循环
            clearTimeout(circle);
            return false;
        }
